 ...
 if(!empty($_GET["x"]))
{
  ...
  while($row = $result->fetch_assoc()){
    $amount[$row['pay_ctg_id']] = $row['amount'];
  }

  $jsonCats = json_encode($categories);
  $jsonSubCats = json_encode($amount);
?>
<!DOCTYPE html>
<html lang="zxx"> 
 ...
 <?php
      ...
      function loadCategories(){
        ...
        for(var i = 0; i < categories.length; i++){
		 ...
		 select.options[i] = new Option('Please Select');  
      ...
      function updateSubCats(){
		...
		document.getElementById("subcatsSelect").value=subcats[catid];
					...
					<form action="check_pay.php" method="post" enctype="multipart/form-data"  onsubmit="return Validate(this);">
						<table align="center" cellpadding="20px" cellspacing="20px" width="100%" height="10%">
							<tr>
								...
								<td  style="padding:10px;border-bottom:10px;margin-right=200px;">
									<select id='categoriesSelect' name="slctcategory" style="border:none;" required>
							...
							<tr>
								...
								<td  style="padding:10px;border-bottom:10px;margin-right=200px;">
									<input type="text" name="amount" id='subcatsSelect' required>
								</td>
							</tr>
							<tr>							
								<td  style="padding:10px;border-bottom:10px;margin-right=200px;">
								<input type="submit" value="Pay" style="width:100%;">
								</td>
