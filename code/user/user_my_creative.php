...
if(!empty($_GET["x"]))
{
	echo "<script>alert('Your File Has Been Deleted!!!');</script>";
}
?>
<html>
<head>
          ...
          <div class="box">
            ...
            <div class="box-body">
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
					<?php
					 		$sql="Select crtv_id,crtv_name,ctg_id,description,file,type,cover,case
when status=0 then 'Pending'
when status=1 then 'Approved'
when status=2 then 'Rejected'
END AS status from tbl_creative where user_id=$uid";
							$res=mysqli_query($con,$sql);
							$n=0;
							while($r=mysqli_fetch_assoc($res))
							{
									$cid=$r['ctg_id'];
									$sq="Select ctg_name from tbl_creative_ctg where ctg_id=$cid";
									...
									elseif($type=='image')
									{
										echo "<td><img width='200' height='200' src='../creative_corner/".$r['file']."'></td>";
									}
									else
									{
									echo "<td><a href='../creative_corner/".$r['file']."' target='_blank'><img width='100' height='100' src='../creative_corner/".$r['cover']."'><br>Click Here</a></td>";
									}
									echo "<td>".$r['status']."</td>
									<td><a href=delete_creative.php?id=".$r['crtv_id'].">
