...
if($fileimg!="")
{
$sql="insert into tbl_books(user_id,book_name,ctg_id,language,auther,description,book,image,status)values('$uid','$name','$catg','$lang','$auther','$des','$file','$fileimg',1)";
}
else
{
$sql="insert into tbl_books(user_id,book_name,ctg_id,language,auther,description,book)values('$uid','$name','$catg','$lang','$auther','$des','$file',1)";
}
mysqli_query($con,$sql);
 echo "<script>var confirm = confirm(\"Your File Has Been Uploaded Successfully!\");
          if(confirm){ 
              window.location='admin_add_books	.php';
           }
		   else{
			   window.location='admin_add_books.php';
		   }
